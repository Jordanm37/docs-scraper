---
title: Configuration - Clawdbot
url: https://docs.clawd.bot/gateway/configuration
---
...
## [​](#strict-config-validation) Strict config validation
...
## [​](#partial-updates-rpc) Partial updates (RPC)
...
## [​](#self-chat-mode-recommended-for-group-control) Self-chat mode (recommended for group control)
...
## [​](#config-includes-$include) Config Includes (`$include`)
...
### [​](#example:-multi-client-legal-setup) Example: Multi-client legal setup
...
### [​](#channels-whatsapp-sendreadreceipts) `channels.whatsapp.sendReadReceipts`
...
### [​](#channels-whatsapp-accounts-multi-account) `channels.whatsapp.accounts` (multi-account)
...
### [​](#group-chat-mention-gating-agents-list[]-groupchat-+-messages-groupchat) Group chat mention gating (`agents.list[].groupChat` + `messages.groupChat`)
...
### [​](#group-policy-per-channel) Group policy (per channel)
...
### [​](#multi-agent-routing-agents-list-+-bindings) Multi-agent routing (`agents.list` + `bindings`)
...
### [​](#tools-agenttoagent-optional) `tools.agentToAgent` (optional)
...
### [​](#messages-inbound) `messages.inbound`
...
### [​](#channels-discord-bot-transport) `channels.discord` (bot transport)
...
### [​](#channels-imessage-imsg-cli) `channels.imessage` (imsg CLI)
...
#### [​](#agents-defaults-clibackends-cli-fallback) `agents.defaults.cliBackends` (CLI fallback)
...
#### [​](#agents-defaults-contextpruning-tool-result-pruning) `agents.defaults.contextPruning` (tool-result pruning)
...
#### [​](#agents-defaults-compaction-reserve-headroom-+-memory-flush) `agents.defaults.compaction` (reserve headroom + memory flush)
...
### [​](#z-ai-glm-4-7-—-provider-alias-support) Z.AI (GLM-4.7) — provider alias support
...
### [​](#local-models-lm-studio-—-recommended-setup) Local models (LM Studio) — recommended setup
...
### [​](#cerebras-glm-4-6-/-4-7) Cerebras (GLM 4.6 / 4.7)
...
### [​](#skills-skills-config) `skills` (skills config)
...
#### [​](#hot-reload-matrix-files-+-impact) Hot reload matrix (files + impact)
...
### [​](#hooks-gateway-webhooks) `hooks` (Gateway webhooks)
...
### [​](#canvashost-lan/tailnet-canvas-file-server-+-live-reload) `canvasHost` (LAN/tailnet Canvas file server + live reload)
...
### [​](#bridge-legacy-tcp-bridge,-removed) `bridge` (legacy TCP bridge, removed)
...
### [​](#discovery-widearea-wide-area-bonjour-/-unicast-dns‑sd) `discovery.wideArea` (Wide-Area Bonjour / unicast DNS‑SD)
...
## [​](#template-variables-2) Template variables
