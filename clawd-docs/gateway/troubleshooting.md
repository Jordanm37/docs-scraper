...
url: https://docs.clawd.bot/gateway/troubleshooting
---
...
### [​](#no-api-key-found-for-provider-“anthropic”) No API key found for provider “anthropic”
...
### [​](#oauth-token-refresh-failed-anthropic-claude-subscription) OAuth token refresh failed (Anthropic Claude subscription)
...
### [​](#”gateway-start-blocked:-set-gateway-mode=local”) ”Gateway start blocked: set gateway.mode=local”
...
### [​](#skill-missing-api-key-in-sandbox) Skill missing API key in sandbox
...
### [​](#address-already-in-use-port-18789) Address Already in Use (Port 18789)
...
### [​](#extra-workspace-folders-detected) Extra Workspace Folders Detected
...
### [​](#”agent-failed-before-reply:-unknown-model:-anthropic/claude-haiku-3-5”) ”Agent failed before reply: Unknown model: anthropic/claude-haiku-3-5”
...
### [​](#pairing-code-not-arriving) Pairing Code Not Arriving
...
### [​](#image-+-mention-not-working) Image + Mention Not Working
...
### [​](#“gateway-won’t-start-—-configuration-invalid”) “Gateway won’t start — configuration invalid”
...
### [​](#i’m-running-on-my-personal-whatsapp-number-—-why-is-self-chat-weird) I’m running on my personal WhatsApp number — why is self-chat weird?
...
### [​](#whatsapp-logged-me-out-how-do-i-re‑auth) WhatsApp logged me out. How do I re‑auth?
...
### [​](#build-errors-on-main-—-what’s-the-standard-fix-path) Build errors on `main` — what’s the standard fix path?
...
### [​](#telegram-block-streaming-isn’t-splitting-text-between-tool-calls-why) Telegram block streaming isn’t splitting text between tool calls. Why?
...
### [​](#discord-doesn’t-reply-in-my-server-even-with-requiremention:-false-why) Discord doesn’t reply in my server even with `requireMention: false`. Why?
...
### [​](#app-crashes-when-granting-permissions-speech/mic) App Crashes when Granting Permissions (Speech/Mic)
...
| Gateway service logs (supervisor) | macOS: `$CLAWDBOT_STATE_DIR/logs/gateway.log` + `gateway.err.log` (default: `~/.clawdbot/logs/...`; profiles use `~/.clawdbot-<profile>/logs/...`) Linux: `journalctl --user -u clawdbot-gateway[-<profile>].service -n 200 --no-pager` Windows: `schtasks /Query /TN "Clawdbot Gateway (<profile>)" /V /FO LIST` |
...
### [​](#browser-not-starting-linux) Browser Not Starting (Linux)
