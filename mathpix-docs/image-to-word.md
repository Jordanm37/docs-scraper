---
title: Image to MS Word
url: https://mathpix.com/image-to-word
---

# Image to MS Word

Mathpix has image to MS Word converter which supports converting printed and handwritten math, text, tables, and chemistry to a Word document.

Works on printed and handwritten images

Works on math, text, tables and chemistry

[Get Started](https://snip.mathpix.com)

## The best image recognition for math and science

Mathpix is the only image to Word converter with high-accuracy OCR features developed specifically for scientific documents like research papers.

### Simple Math

![Simple Equation]()

### Complex Math

![Complex Equation]()

### Handwritten Math

![Handwritten Equation]()

### Mixed Text and Math

![Mixed Text and Math]()

* ## How to convert images to MS Word on mobile

  1. Use Snip mobile app to take a photo of the image.

  2. Crop the part of an image you want to digitize and scan it.

  3. Sync to desktop and copy your equation in MS Word format. You can also export it as a .docx file.

  [Get the mobile app](/mobile-downloads)

* ## How to convert images to MS Word on desktop

  1. Use Snip to take a screenshot of the image.

  2. Click "Copy MS Word" to copy the result to your clipboard.

  3. Paste directly into your MS Word document.

  [Get the desktop app](/desktop-downloads)

FOR STUDENTS

## Snip spps

Instantly digitize printed and handwritten notes with Snip and export to MS Word.

![Equation to Word]()

![VS Code feature details small]()

[Learn more](/snip)

## Read Image to Word related posts on our blog

![]()

2021-06-01

### Improved performance on Windows Snip

We have made significant upgrades to the performance of the Windows apps, including bug fixes and stability improvements.

[Read more](/blog/performance-improvements-windows)

![]()

2020-09-02

### Major improvements to MS Word compatibility

Mathpix Snip is more compatible with Microsoft Word than ever with a new MathML format enabling better multiline equations, summations, and integrals in Word.

[Read more](/blog/improved-compatibility-msword)

![]()

2020-03-25

### How to insert an equation into Microsoft Word with Mathpix Snip

Skip the difficult to use equation editor in Word and instead go directly from screenshot to formatted equation in your document...

[Read more](/blog/insert-equations-into-microsoft-word)